...
void setupFileMenu(QMenuBar* myMenuBar, glShaderWindow* glWindow, QApplication *myApp)
{
    ...
    QAction* openAnimationAction = new QAction(myMenuBar->tr("&Load bvh file"), fileMenu);
    openAnimationAction->setStatusTip(myMenuBar->tr("&Opens a new animation file"));
    glWindow->connect(openAnimationAction, SIGNAL(triggered()), glWindow, SLOT(openNewBvhFile()));
    fileMenu->addAction(openAnimationAction);
     // Load new weight file
    QAction* openWeightAction = new QAction(myMenuBar->tr("&Load weight file"), fileMenu);
    openWeightAction->setStatusTip(myMenuBar->tr("&Opens a new weight file"));
    glWindow->connect(openWeightAction, SIGNAL(triggered()), glWindow, SLOT(openNewWeightFile()));
    fileMenu->addAction(openWeightAction);
